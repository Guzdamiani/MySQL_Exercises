...
select nombre_depto from departamentos; /* si se quieren mostrar todos, independeintemente de si se repiten nombres(ej: vetnas para barcelona y ventas para madrid)*/
select distinct nombre_depto from departamentos; /* sin repetir*/
...
select sal_emp as Salario, comision_emp as Comision from empleados where id_depto = 2000;
...
select nombre, (sal_emp + comision_emp + 500) as 'Total a pagar' from empleados where id_depto = 3000;
...
select d.id_depto, d.nombre_depto as 'Nombre depto', COUNT(e.id_emp) as 'Nº Empleados'
